...
This sets `ping` as the default command to run in a container created from this
...
1. Open your Dockerfile and modify the `ENTRYPOINT` instruction to include 2
...
2. If `CMD` and `ENTRYPOINT` are both specified in a Dockerfile, tokens listed in
`CMD` are used as default parameters for the `ENTRYPOINT` command. Add a `CMD`
...
This time, you should see a ping to 8.8.8.8. Explain the di erence in behavior
between these two last containers.


In this exercise, we encountered the Dockerfile commands `CMD` and `ENTRYPOINT`.
These are useful for defining the default process to run as PID 1 inside the
container right in the Dockerfile, making our containers more like executables
