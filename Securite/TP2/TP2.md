...
1) On observe avec `lsof -i` que Apache2 est lancé:  
  ![Processus Apache2 affichés par lsof -i](image.png)
...
![Page d'accueil Apache2 sur localhost](image-1.png)
&nbsp;  

2) J'ai servi la page `xss_cookies.php` et j'ai entré `<script> alert('message') </script>` dans le champ de saisie:  
![Alerte JavaScript affichant 'message' suite à l'injection XSS](image-2.png)
...
![Alerte JavaScript affichant les cookies du navigateur](<Capture d'écran 2026-02-05 144422.png>)
...
![Cookies affichés via injection XSS](image-3.png)
...
![Cookies visibles dans Chrome DevTools](image-4.png)
&nbsp;  
...
![Script XSS neutralisé et affiché comme texte après htmlentities()](image-5.png)
&nbsp;  

8) Dans `messages.txt`, on trouve bien les entités HTML au lieu des caractères spéciaux:  
![Contenu de messages.txt avec entités HTML échappées](image-6.png)
...
![Page dom.html affichant 'essai' depuis le paramètre URL](image-7.png)
&nbsp;  
...
![Script affiché comme texte encodé, non exécuté](image-8.png)
...
12) Pour faire fonctionner la faille, on peut utiliser la fonction `decodeURIComponent()` pour décoder les caractères spéciaux dans l'URL. En modifiant le script dans `dom.html` comme suit:  
    ...
    ![Injection XSS réussie avec decodeURIComponent()](image-9.png)
...
![Création de l'utilisateur mock1 avec useradd et passwd](image-10.png)
...
![Page de manuel de crypt montrant le format d'empreinte](image-11.png)
&nbsp;  
...
![Entrée de mock1 dans /etc/shadow avec hash yescrypt](image-12.png)
...
![Génération du hash MD5 avec openssl passwd -1](image-13.png)
![Édition de /etc/shadow pour ajouter le hash de mock2](image-14.png)
Ce mot de passe fonctionne bien (avec la commande `su - mock2`):  
![Connexion réussie avec su - mock2](image-15.png)
...
![Mot de passe j1l2t3 trouvé dans le dictionnaire de John](image-16.png)
...
![John trouvant le mot de passe j1l2t3 en 59 secondes](image-17.png)
...
![Changement du mot de passe en 'wx' non présent dans le dictionnaire](image-18.png)
...
![John ne trouvant pas le mot de passe 'wx' avec le dictionnaire](image-19.png)
...
![Configuration john.conf avec mode incremental essai sur 2 caractères](image-21.png)
...
![John trouvant le mot de passe 'wx' en 1 minute avec recherche incrémentale](image-22.png)
...
![John en cours d'exécution pour recherche sur 4 caractères](image-23.png)  
...
![Configuration john.conf avec mode incremental essai sur 4 caractères](image-24.png)
...
![Modification de /etc/shadow après changement de mot de passe](image-25.png)
&nbsp;  
...
![Permissions du fichier /etc/shadow (-rw-r-----)](image-26.png)
&nbsp;  
...
![Permissions du programme /usr/bin/passwd avec bit setuid (-rwsr-xr-x)](image-27.png)
...
![Commande id montrant uid=0(root) dans le shell setuid](image-33.png)
...
![Fichier créé appartenant à root depuis le shell setuid](image-34.png)
